    ...
    with _write_lock:
        conn.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS extensions (
                ...
                source TEXT DEFAULT 'armada',
                created_at TEXT NOT NULL DEFAULT (datetime('now'))
            );

            CREATE TABLE IF NOT EXISTS extension_assignments (
                extension_id TEXT NOT NULL REFERENCES extensions(id) ON DELETE CASCADE,
                project_label_id TEXT,
                enabled INTEGER DEFAULT 1,
                PRIMARY KEY (extension_id, project_label_id)
            );
        """)
        conn.commit()
    _migrate()
...
def scan_builtin_extensions():
    """Discover built-in skills, hooks, and plugins from the source tree.
    Idempotent: inserts new extensions, updates descriptions on existing ones."""
    from pathlib import Path
    repo_root = Path(__file__).parent.parent.parent

    skills_dir = repo_root / "skills"
    hooks_dir = repo_root / "armada_ai" / "hooks"
    ...
    if skills_dir.is_dir():
        for d in sorted(skills_dir.iterdir()):
            ...
            if not skill_md.is_file():
                continue
            ext_id = d.name
            desc = _scan_skill_description(str(skill_md))
            extensions.append({
                "id": ext_id, "type": "skill", "name": d.name,
                "description": desc, "source": "armada",
            })
    ...
    if hooks_dir.is_dir():
        for f in sorted(hooks_dir.iterdir()):
            ...
            desc = _scan_skill_description(str(f))
            extensions.append({
                "id": ext_id, "type": "hook", "name": f.name,
                "description": desc, "source": "armada",
            })
    ...
    if plugins_dir.is_dir():
        for f in sorted(plugins_dir.iterdir()):
            ...
            extensions.append({
                "id": ext_id, "type": "plugin", "name": f.name,
                "description": "", "source": "armada",
            })

    _upsert_extensions(extensions)
    return extensions


def _upsert_extensions(ext_list: list[dict]):
    def _do():
        conn = _get_conn()
        for ext in ext_list:
            conn.execute(
...
def list_extensions(project_label_id: str | None = None) -> list[dict]:
    """List all extensions with their install status.
    If project_label_id is given, only show extensions relevant to that project."""
    conn = _get_conn()
    rows = conn.execute("""
        SELECT e.*, ea.project_label_id as assigned_project, ea.enabled
        FROM extensions e
        LEFT JOIN extension_assignments ea ON e.id = ea.extension_id
        ORDER BY e.type, e.name
    """).fetchall()
    return [dict(r) for r in rows]


def install_extension(extension_id: str, project_label_id: str | None = None):
    """Install an extension globally (project_label_id=None) or to a project."""
    def _do():
        conn = _get_conn()
        conn.execute(
            "INSERT OR REPLACE INTO extension_assignments (extension_id, project_label_id, enabled) "
            "VALUES (?, ?, 1)",
            (extension_id, project_label_id),
        )
        conn.commit()
    _retry(_do, write=True)


def remove_extension_assignment(extension_id: str, project_label_id: str | None = None):
    """Remove an extension assignment (uninstall)."""
    def _do():
        conn = _get_conn()
        conn.execute(
            "DELETE FROM extension_assignments WHERE extension_id = ? AND project_label_id IS ?",
            (extension_id, project_label_id),
        )
        conn.commit()
    _retry(_do, write=True)
...
def get_project_extensions(project_label_id: str) -> list[dict]:
    """List extensions installed for a specific project (enabled assignments)."""
    conn = _get_conn()
    rows = conn.execute("""
        SELECT e.* FROM extensions e
        JOIN extension_assignments ea ON e.id = ea.extension_id
        WHERE ea.project_label_id = ? AND ea.enabled = 1
        ORDER BY e.type, e.name
    """, (project_label_id,)).fetchall()
    return [dict(r) for r in rows]
